...
const FWOutcomeEngine = (() => {
  ...
  function narrate(verdict, recorded, alignment, checksRun) {
    const blind = checksRun === 0 ? ' Closed without checking any record source first.' : '';
    if (alignment === 'UNSCORABLE') {
      return `No answer key is held for this case's signals, so this verdict cannot be checked against anything.${blind}`;
    }
    if (alignment === 'AMBIGUOUS') {
      return `The records account for ${recorded.explained} of ${recorded.total} signals on this case and leave the rest unaccounted for. Neither closing verdict is unreasonable on that evidence, so this one is not scored either way.${blind}`;
    }
    if (verdict === 'CONFIRMED' && alignment === 'ALIGNED') {
      return `No documented benign cause exists for any of the ${recorded.total} signals on this case. That is consistent with the call, though an absence of explanation is still not proof of an act.${blind}`;
    }
    if (verdict === 'CONFIRMED' && alignment === 'OVERCALLED') {
      return `The simulation held a documented benign cause for all ${recorded.total} signals on this case (${recorded.causes.join('; ')}). Escalating it was an over-call against the record.${blind}`;
    }
    if (verdict === 'FALSE_POSITIVE' && alignment === 'ALIGNED') {
      return `Every signal on this case had a documented benign cause on record (${recorded.causes.join('; ')}), consistent with clearing it.${blind}`;
    }
    return `No documented benign cause existed for any of the ${recorded.total} signals on this case, yet it was cleared. That is an under-call against the record — which is not the same as saying something happened.${blind}`;
  }
  ...
  function recordVerdict(state, mo, status) {
    ...
    const entry = {
      ...
      narrative: isVerdict
        ? narrate(status, recorded, alignment, inv.checksRun)
